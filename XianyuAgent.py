...
class XianyuReplyBot:
    ...
    def _init_rag(self):
        """初始化 RAG 检索器，失败时降级为 None 不阻断主流程"""
        try:
            from xianyu_rag_agent.rag.retriever import RAGRetriever
            from context_manager import ChatContextManager
            self.rag = RAGRetriever(context_manager=ChatContextManager())
            logger.info("RAG 知识库检索器已就绪")
        except Exception as e:
            logger.warning(f"RAG 检索器初始化失败，将降级运行（无知识库增强）: {e}")
            self.rag = None
    ...
    def _init_agents(self):
        ...
        self.agents = {
            ...
            'tech': TechAgent(self.client, self.tech_prompt, self._safe_filter, self.rag),
...
class BaseAgent:
    ...
    def generate(self, user_msg: str, item_desc: str, context: str, bargain_count: int = 0, item_id: str = None) -> str:
        """生成回复模板方法"""
...
class TechAgent(BaseAgent):
    ...
    def generate(self, user_msg: str, item_desc: str, context: str, bargain_count: int=0, item_id: str=None) -> str:
        ...
        # RAG 检索：按当前商品范围召回店铺通用 + 该商品专属知识
        if self.rag is not None:
            try:
                knowledge = self.rag.search(user_msg, item_id=item_id, top_k=3)
                if knowledge:
                    messages[0]['content'] += "\n▲知识库参考：\n" + knowledge
                    logger.debug("RAG 命中知识，已注入 TechAgent prompt")
            except Exception as e:
                logger.warning(f"RAG 检索异常，跳过注入: {e}")
