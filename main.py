    ...
    async def control_loop(self):
        ...
        while True:
            try:
                ...
                for cmd in pending:
                    await self._execute_control(cmd)
            except Exception as e:
                logger.error(f"控制命令轮询出错: {e}")
            await asyncio.sleep(poll_interval)

    async def _execute_control(self, cmd):
        """执行单条控制命令并回写结果。"""
        command = cmd.get("command")
        cmd_id = cmd.get("id")
        logger.info(f"收到控制命令: {command} (id={cmd_id})")
        try:
    ...
    async def main(self):
        while True:
            try:
                ...
                if getattr(self, "control_task", None):
